...
export function Header({ activePlatform, onPlatformChange, viewMode, onViewModeChange, onAddClick }: HeaderProps) {
  ...
  return (
    <header className="h-16 sticky top-0 z-20 bg-white/95 backdrop-blur-lg border-b border-slate-200 flex items-center px-4 gap-3">
      {/* Search — matches mockup input styling */}
      <form onSubmit={handleSearch} className="flex-1 max-w-xl relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Cari dengan ide, bukan keyword..."
            className="w-full bg-slate-100 border-2 border-transparent rounded-2xl pl-12 pr-4 py-2.5 text-sm focus:outline-none search-glow focus:border-blue-500 focus:bg-white transition"
          />
          {query && (
            <button
              type="button"
